    ...
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-m-hairline pb-4">
          ...
          <button
            onClick={() => setIsWaiterModalOpen(false)}
            className="p-2 rounded-full text-m-text-muted hover:text-m-text hover:bg-m-surface-raised transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {!justCalled && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3.5 rounded-full brand-cta disabled:opacity-50 font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
                <>
