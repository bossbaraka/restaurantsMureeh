...
export const CustomerHeader: React.FC = () => {
  ...
  const initialLetter = restName.charAt(0) || restNameEn.charAt(0) || '•';
  ...
  return (
    <>
      <header
        ref={stickyBandRef}
        className="sticky z-30 bg-m-bg/95 backdrop-blur-md border-b border-m-hairline px-4 sm:px-6 pt-[max(0.625rem,env(safe-area-inset-top))] pb-2.5 transition-all"
        style={{
      ...
      >
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-3">
          ...
          {/* Desktop actions. Secondary targets stay mounted; the cart is the
              one filled control. */}
          <div className="hidden sm:flex items-center gap-2">
            <button
              onClick={() => openCustomerGuide()}
              className="flex items-center justify-center w-10 h-10 rounded-full bg-m-surface hover:bg-m-surface-raised text-m-text border border-m-hairline transition-all active:scale-95 cursor-pointer"
          ...
          <div className="flex sm:hidden items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-full bg-m-surface border border-m-hairline text-m-text hover:text-[var(--m-brand-on-surface)] transition-all cursor-pointer"
