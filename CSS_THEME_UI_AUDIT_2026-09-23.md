...
## ✅ COMMITS 4+5 (d87b5ad، 6870009) — تنفيذ البندين 1 و2

### Commit 4 (d87b5ad) — حذف `LuxuryWelcomeScreen` (البند 1)
- **الحذف:** `LuxuryWelcomeScreen.tsx` (1623 سطر) + `welcomeScreen.test.tsx` (45 اختباراً) + كتلة `.welcome-*` في index.css (**541 سطراً: 249–789**، شاملة توكنز `:root` `--welcome-*` — تحقق قبل الحذف أن كل مُحدد في النطاق welcome-scoped وأن التوكنز لا تُستهلك خارجه) + تعليق `--welcome-canvas` القديم على `--mureeh-canvas`.
- **اختبارات محدّثة** (كانت تفحص مصدر الملف المحذوف): `appearanceIsolation` (قائمتا ملفات + اختبار welcome-purity)، `customerTokenGuard` (الإعفاء + حلقة canvas)، `logoFraming` (تأكيد welcome).
- **أثر بصري: صفر** — المسار كان غير قابل للوصول إنتاجياً.

### Commit 5 (6870009) — توحيد ألوان الحالة (البند 2)
- `semanticTokens.ts`: توكن `--m-info` (+قنوات) يكمل الرباعية، وأربع ظلال `--m-*-strong` (+قنوات) بدرجات -400 — **platform-fixed** بقيم مطابقة بايت-ببايت للكلاسات السابقة (blue-500/400…)، مع توثيق سبب عدم اشتقاقها من تجاوزات المستأجر (حماية التباين).
- `formatting.ts getOrderStatusConfig`: الشارات والنقاط تستهلك التوكنز الآن عبر arbitrary classes **بـ fallbacks مضمّنة** — الحل العملية لمشكلة مشاركة الدالة مع المنصة خارج الـ scope (القيمة المتدهورة = نفس القيمة السابقة حرفياً). `SERVED` يبقى zinc عمداً (محايد، ليس حالة brand).
- **حلّ التحفظان المسجلان سابقاً:** الـ fallbacks المضمنة تُبقي المنصة آمنة، و`--m-info` أُضيف كتوكن platform-fixed بنفس نمط success/warning/error (بلا مساس بعقد الخادم).
- **أثر بصري: صفر** — تحقق بالحزمة المبنية (قواعد `rgb(var(--m-*-rgb,…))` مُولَّدة كاملة).

### التحقق (الجولتان)
`lint` 0 errors (251 ملفاً بعد الحذفين) · `tsc -b` PASS · build PASS · vitest: **90 ملفاً / 1297 ناجحاً** (1342 − 45 المحذوفة) — نفس الفشلين البيئيين (Prisma sandbox) فقط، صفر انحدارات.
